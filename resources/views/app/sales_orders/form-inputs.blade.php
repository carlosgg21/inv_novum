...
<div class="row">
    ...
    <div class="form-group dynamic-element" style="display:none">
        <div class="row">
            <div class="col-sm-3">
                <select class="form-control form-control-sm product-select" name="products[]">
                    ...
                    <option value="{{ $product->id }}" data-price="{{ $product->unit_price }}"
                        data-description="{{ $product->description }}" data-qty="{{ $product->qty }}">{{ $product->name
                        }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="col-sm-2">
                <input type="number" class="form-control form-control-sm qty-input" min="1"  name="qty[]" placeholder="Qty"
    ...
    <div class="container-fluid">
        ...
        <div class="form-horizontal">
            ...
            <p class="text-center add-one">Add Product to Sales Order Datails</p>
    ...
    <x-inputs.group class="col-sm-12 col-lg-3">
        ...
        <x-inputs.number name="taxes" label="Taxes"  :value="old('taxes', ($editing ? $salesOrder->taxes : ''))"
            max="255" step="1" placeholder="Taxes"></x-inputs.number>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12 col-lg-3">
        <label for="discount">Discount </label>
        <x-inputs.number name="discount" label="Discount"
            :value="old('discount', ($editing ? $salesOrder->discount : ''))" max="255" step="0.01"
            placeholder="Discount"></x-inputs.number>
    </x-inputs.group>

    <x-inputs.group class="col-sm-3">
        <label for="miscellaneous">Miscellaneous </label>
        <x-inputs.number name="miscellaneous" label="Miscellaneous"
            :value="old('miscellaneous', ($editing ? $salesOrder->miscellaneous : ''))" max="255" step="0.01"
            placeholder="Miscellaneous"></x-inputs.number>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12 col-lg-3">
        <label for="freight">Freight </label>
        <x-inputs.number name="freight" label="Freight" :value="old('freight', ($editing ? $salesOrder->freight : ''))"
            max="255" step="0.01" placeholder="Freight"></x-inputs.number>
    </x-inputs.group>
    <x-inputs.group class="col-sm-12">
        <div class="text-right">
            <label for="order_total">Order Total </label>
            <x-inputs.number name="order_total" class="text-right"
                :value="old('order_total', ($editing ? $salesOrder->order_total : ''))" max="255" step="0.01"
                placeholder="Order Total" disabled></x-inputs.number>
            </div>
</x-inputs.group>
    ...
    <x-inputs.group class="col-sm-12">
        <x-inputs.textarea name="notes" label="Notes" maxlength="255">{{ old('notes', ($editing ? $salesOrder->notes :
            ''))
            }}</x-inputs.textarea>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12">
        <x-inputs.textarea name="internal_notes" label="Internal Notes" maxlength="255">{{ old('internal_notes',
            ($editing ? $salesOrder->internal_notes :
            '')) }}</x-inputs.textarea>
    </x-inputs.group>
